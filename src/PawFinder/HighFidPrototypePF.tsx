...
export const HighFidPrototypePF = ({data, textColor}:any) => {
  ...
  return (
    <div className="hf_prototype_container_PF">
      ...
      <div>
        <img className="hf_prototype_image_container_PF" src={image}/>
      </div>
      <p className={`${textColor}65 font_size_desc`}>{imageDesc}<a className="hf_prototype_link_PF" href={link}>{linkText}</a></p>
    </div>
  )
};